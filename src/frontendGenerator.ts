...
async function extractProgramID(workspaceRoot: string): Promise<string | null> {
  try {
    ...
    const match = content.match(/\[programs\.devnet\]\s*\n\s*([a-zA-Z0-9]+)\s*=\s*"([a-zA-Z0-9]+)"/)
    if (match) {
      return match[2]
    }

    // Fallback: look for any program ID line
    const idMatch = content.match(/^[a-zA-Z0-9_]+\s*=\s*"([a-zA-Z0-9]{32,44})"/m)
...
function parseFrontendResponse(raw: string): FrontendGenerationResponse {
  ...
  try {
    ...
    try {
      const repaired = slice.replace(/"content"\s*:\s*"([\s\S]*?)"/g, (match) => {
        return match.replace(/\n/g, '\\n').replace(/\r/g, '\\r').replace(/\t/g, '\\t')
      })
      parsed = JSON.parse(repaired) as FrontendGenerationResponse
    } catch (err2) {
      throw new Error('AI response JSON is malformed and could not be repaired.')
    }
  }
  ...
  for (const file of parsed.files) {
    ...
    if (normalized.split(path.sep).includes('..')) {
      throw new Error(`Refusing to write file with path traversal: ${file.path}`)
    }
  }
  ...
  try {
    ...
    if (!programId) {
      const input = await vscode.window.showInputBox({
        prompt: '📌 Enter the Program ID (from Anchor.toml or deployed program)',
        placeHolder: 'e.g., 11111111111111111111111111111111',
        ignoreFocusOut: true,
        validateInput: (val) =>
          val.trim().length >= 32 ? undefined : 'Program ID must be at least 32 characters.',
      })
    ...
    await vscode.window.withProgress(
      ...
      async (progress) => {
        progress.report({ message: 'Crafting React app with Anchor client...' })
    ...
    const message = error instanceof Error ? error.message : 'Unknown frontend generation error'
    void vscode.window.showErrorMessage(`❌ Frontend generation failed: ${message}`)
  }
}
 ...
 */
function writeGeneratedFiles(workspaceRoot: string, files: Array<{ path: string; content: string }>): void {
  const MAX_FILE_BYTES = 1_000_000 // 1 MB for frontend files

  for (const file of files) {
    // Validate size
    if (Buffer.byteLength(file.content, 'utf-8') > MAX_FILE_BYTES) {
      throw new Error(`File too large: ${file.path} exceeds ${MAX_FILE_BYTES} bytes`)
    }
